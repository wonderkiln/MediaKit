...
open class MKCropView: UIView, UIScrollViewDelegate, UIGestureRecognizerDelegate, MKCropRectViewDelegate {
    ...
    open var margins: CGSize = CGSize(width: 20, height: 20) {
        didSet {
            layoutIfNeeded()
        }
    }
    ...
    fileprivate let cropRectView = MKCropRectView()
    ...
    fileprivate func initialize() {
        ...
        rotationGestureRecognizer = UIRotationGestureRecognizer(target: self, action: #selector(MKCropView.handleRotation(_:)))
    ...
    open override func layoutSubviews() {
        ...
        if !resizing {
            layoutMKCropRectViewWithCropRect(scrollView.frame)
    ...
    open func resetCropRectAnimated(_ animated: Bool) {
        ...
        layoutMKCropRectViewWithCropRect(scrollView.bounds)
    ...
    fileprivate func showOverlayView(_ show: Bool) {
        let color = show ? UIColor.black.withAlphaComponent(0.7) : UIColor.clear
    ...
    fileprivate func layoutMKCropRectViewWithCropRect(_ cropRect: CGRect) {
    ...
    fileprivate func zoomToCropRect(_ toRect: CGRect, shouldCenter: Bool, animated: Bool, completion: (() -> Void)? = nil) {
        ...
        UIView.animate(withDuration: duration, delay: 0.0, options: .beginFromCurrentState, animations: { [unowned self] in
            ...
            self.layoutMKCropRectViewWithCropRect(cropRect)
        }) { finished in
            completion?()
        }
    }
    
    fileprivate func cappedCropRectInImageRectWithMKCropRectView(_ cropRectView: MKCropRectView) -> CGRect {
    ...
    // MARK: - MKCropView delegate methods
    func cropRectViewDidBeginEditing(_ view: MKCropRectView) {
        resizing = true
    }
    
    func cropRectViewDidChange(_ view: MKCropRectView) {
        let cropRect = cappedCropRectInImageRectWithMKCropRectView(view)
        layoutMKCropRectViewWithCropRect(cropRect)
